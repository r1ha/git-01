...
assert_step_exists() {
  ...
  if [[ $? -ne 0 ]] ; then
    printf "${RED}value '${step}' not supported. Supported values are: "
    printf "${STEPS[@]}"
    printf "${NC}\n"
...
assert_is_git() {
  ...
  if [[ $? -ne 0 ]] ; then
    printf "${RED}Not running inside a git repository${NC}\n";
    exit 3
  fi;
}
...
exec_step() {
  ...
  if [ ${DONE} -gt 0 ] && [ "${FORCE}" != "1" ]; then
    echo "Step ${STEP} already applied. Use --force to apply it again."
  fi;
